plot_fit <- function(dat,run,pop,u,moving_avg = FALSE,n_smpls = 1000){
    plot_outcome(dat$samples$trajectories$state,dat$data,"cases",dat$samples$pars[,"phi_cases"],by_age = T,moving_avg = moving_avg)
    ggsave(paste0("output/cases_by_age_fit",ifelse(moving_avg,"_moving_avg",""),run,".pdf"),width = 8, height = 6.4)
    plot_outcome(dat$samples$trajectories$state,dat$data,"hosps",by_age = T,moving_avg = moving_avg)
    ggsave(paste0("output/hosps_by_age_fit",ifelse(moving_avg,"_moving_avg",""),run,".pdf"),width = 4, height = 8)
    plot_outcome(dat$samples$trajectories$state,dat$data,"deaths",by_age = T,moving_avg = moving_avg)
    ggsave(paste0("output/deaths_by_age_fit",ifelse(moving_avg,"_moving_avg",""),run,".pdf"),width = 4, height = 8)
    plot_sero(dat$samples$trajectories$state,dat$data,pop[,.(population = sum(total)),by = .(age_group)],by_age = T)
    ggsave(paste0("output/sero_by_age_fit",run,".pdf"),width = 4, height = 8)
    plot_outcome(dat$samples$trajectories$state,dat$data,"cases",dat$samples$pars[,"phi_cases"],moving_avg = moving_avg)
    ggsave(paste0("output/cases",ifelse(moving_avg,"_moving_avg",""),run,".pdf"),width = 4, height = 2.7)
    plot_outcome(dat$samples$trajectories$state,dat$data,"hosps",moving_avg = moving_avg)
    ggsave(paste0("output/hosps",ifelse(moving_avg,"_moving_avg",""),run,".pdf"),width = 4, height = 2.7)
    plot_outcome(dat$samples$trajectories$state,dat$data,"deaths",moving_avg = moving_avg)
