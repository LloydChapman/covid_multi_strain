plot_fit <- function(dat,run,pop,u,burnin = NULL,moving_avg = FALSE,n_smpls = 1000){
    if (is.null(burnin)){
        burnin <- round(nrow(dat$samples$pars)/10)
    }
    
    plot_outcome(dat$samples$trajectories$state,dat$data,"cases",dat$samples$pars[,"phi_cases"],by_age = T,burnin = burnin,moving_avg = moving_avg)
    ggsave(paste0("output/cases_by_age_fit",ifelse(moving_avg,"_moving_avg",""),run,".pdf"),width = 8, height = 6.4)
    plot_outcome(dat$samples$trajectories$state,dat$data,"hosps",by_age = T,burnin = burnin,moving_avg = moving_avg)
    ggsave(paste0("output/hosps_by_age_fit",ifelse(moving_avg,"_moving_avg",""),run,".pdf"),width = 4, height = 8)
    plot_outcome(dat$samples$trajectories$state,dat$data,"deaths",by_age = T,burnin = burnin,moving_avg = moving_avg)
    ggsave(paste0("output/deaths_by_age_fit",ifelse(moving_avg,"_moving_avg",""),run,".pdf"),width = 4, height = 8)
    plot_sero(dat$samples$trajectories$state,dat$data,pop[,.(population = sum(total)),by = .(age_group)],by_age = T,burnin = burnin)
    ggsave(paste0("output/sero_by_age_fit",run,".pdf"),width = 4, height = 8)
    plot_outcome(dat$samples$trajectories$state,dat$data,"cases",dat$samples$pars[,"phi_cases"],burnin = burnin,moving_avg = moving_avg)
    ggsave(paste0("output/cases",ifelse(moving_avg,"_moving_avg",""),run,".pdf"),width = 4, height = 2.7)
    plot_outcome(dat$samples$trajectories$state,dat$data,"hosps",burnin = burnin,moving_avg = moving_avg)
    ggsave(paste0("output/hosps",ifelse(moving_avg,"_moving_avg",""),run,".pdf"),width = 4, height = 2.7)
    plot_outcome(dat$samples$trajectories$state,dat$data,"deaths",burnin = burnin,moving_avg = moving_avg)
    ...
    names(ttls) <- vrble
    p_list <- plot_outcome_by_age(dat$samples$trajectories$state,vrble,dat$samples$pars[,"phi_cases"],ttls,n_smpls,burnin = burnin)
    ...
    base <- pars$base
    res1 <- plot_transmission_rate(dat$samples$pars,base$beta_type,base$beta_date,base$dt,max(dat$data$day_end),burnin = burnin)
    p <- res1$p
    res2 <- plot_transmission_rate(dat$samples$pars[,c(1,3,5:14)],base$beta_type,base$beta_date[c(1,3,5)],base$dt,max(dat$data$day_end),burnin = burnin)
    ...
    plot_posteriors(dat$samples$pars,u,priors,pars_min,pars_max,burnin = burnin)
    ggsave(paste0("output/par_posteriors",run,".pdf"),width = 6,height = 6)
    
    p2 <- plot_pairwise_correlation(dat$samples$pars,u,burnin = burnin)
