...
vaccination_parameters <- function(N_tot,
    ...
    ret <- Map(function(value, name)
        build_rel_param(value, n_groups, n_vacc_classes, name),
...
plot_particle_filter <- function(history, true_history, times, obs_end = NULL) {
    ...
    for (i in 1:n_age){
        ...
        matplot(times, t(history[i + n_age*n_vax, ,-1]), type = "l", # Offset to access numbers in age compartment
                xlab = "", ylab = "", yaxt="none", main = paste0("Age ", contact$demography$age.group[i]),
                col = alpha(cols[["E"]],0.01), lty = 1, ylim=range(true_history[i + 12 + seq(n_age*n_vax,4*n_age*n_vax,by = n_age*n_vax), ,-1]))
        ...
        #           col = cols[2:5])
        points(times[1:obs_end], true_history[i + 12 + n_age*n_vax, ,-1], pch = 19,
                  col = cols[2])
...
plot_hosps_and_deaths_age <- function(incidence_modelled, incidence_observed, times, n_age, n_vax){
    par(mfrow = c(2,4), oma=c(2,3,0,0))
    idx1 <- 9*n_age*n_vax + 1
    for (i in 1:5){
        par(mar = c(3, 4, 2, 0.5))
