...
beta_date_cntfctl_list <- replicate(10,beta_date,F)
...
schedule_cntfctl_list <- replicate(10,schedule,F)
...
# Run simulations
for (i in 9:10){ # seq_along(beta_date_cntfctl_list)){
...
ggsave(paste0("output/cntfctl_hosps_and_deaths_vax",run,".pdf"),plot_grid(pp,l,nrow = 2,rel_heights = c(1,0.1)),width = 9,height = 3.5)
...
ggsave(paste0("output/cntfctl_hosps_and_deaths_booster",run,".pdf"),plot_grid(pp1,l1,nrow = 2,rel_heights = c(1,0.1)),width = 9,height = 3.5)
