...
cases <- add_noise(cases,0.1)
rownames(cases) <- "cases"
cases_non_variant <- add_noise(cases_non_variant,0.1)
rownames(cases_non_variant) <- "cases_non_variant"
...
# Make sure that simulated non-variant cases don't exceed total cases
data_raw$cases_non_variant <- pmin(data_raw$cases_non_variant,data_raw$cases)
...
beta_value_list <- list(name = c("beta1","beta2"), initial = c(0.08, 0.04),
                  min = rep(0,2), max = rep(Inf,2), discrete = rep(F,2),
                  prior = list(function(x) dgamma(x, shape = 1, scale = 1, log = TRUE),
                               function(x) dgamma(x, shape = 1, scale = 1, log = TRUE)))
...
proposal <- diag(c(rep(0.01^2,3),rep(2^2,3)))
