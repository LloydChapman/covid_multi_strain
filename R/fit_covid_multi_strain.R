fit_covid_multi_strain <- function(data_raw,pars,u,n_iters,run,deterministic = TRUE,Rt = FALSE,thinning = 1){
    ...
    # # pars_init <- c(beta_value_list$initial,rel_strain_transmission$initial,start_date$initial,strain_seed_date$initial)
    # # priors <- c(beta_value_list$prior,rel_strain_transmission$prior,replicate(2,function(x) 0))
    # pars_init <- c(beta_value_list$initial,
    ...
    # discrete <- c(rep(F,length(pars_init)))
    # # names(pars_init) <- names(priors) <- names(pars_min) <- names(pars_max) <- names(discrete) <- c(beta_value_list$name,"rel_strain_transmission","start_date","strain_seed_date")
    # names(pars_init) <- names(priors) <- names(pars_min) <- names(pars_max) <- 
    #     names(discrete) <- 
    ...
    # Trace plots
    # par(mfrow = c(ceiling(length(pars_init)/2),2))
