...
compare <- function(state, observed, pars = NULL){
    ...
    # incidence_modelled <- prev_state[7:14,,] - state[7:14,,]
...
n_particles <- 100
filter <- particle_filter$new(data, gen_seird_age, n_particles, compare)
...
parameter_transform <- function(pars,dt,S_ini,E_ini,I_ini,sigma,m,p_death,N_age){
    beta <- pars[["beta"]]
    gamma <- pars[["gamma"]]
...
transform <- function(pars){
    parameter_transform(pars,dt = dt,S_ini = S_ini,E_ini = E_ini,I_ini = I_ini,sigma = 0.5,
                        m = transmission,p_death = p_death,N_age = N_age)
}

# proposal_matrix <- matrix(0.01^2,nrow = 1,ncol = 1)
proposal_matrix <- matrix(c(0.01^2,0,0,0.01^2),nrow = 2,ncol = 2,byrow = TRUE)
mcmc_pars <- pmcmc_parameters$new(# list(beta = beta),
...
plot_particle_filter(pmcmc_run$trajectories$state[,101:501,], true_history, data_raw$day)

# Plot sampled incidence curves on top of data
plot_incidence(pmcmc_run$trajectories$state[,101:501,],incidence,data_raw$day)
...
mcmc_pars1 <- pmcmc_parameters$new(#list(beta = beta),
                                   list(beta = beta,gamma = gamma),
...
# Plot sampled trajectories
plot_particle_filter(pmcmc_run1$trajectories$state, true_history, data_raw$day)

# Plot sampled incidence curves on top of data
plot_incidence(pmcmc_run1$trajectories$state[,101:501,],incidence,data_raw$day)
...
filter <- particle_filter$new(data, gen_seird_age, n_particles, 
...
filter <- particle_filter$new(data_age, gen_seird_age, n_particles, 
...
save.image("test_examples/seird_age_output.RData")
